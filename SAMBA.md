...
## 📁 Compartilhamento personalizado: `[ti]`

Exemplo de uma seção `[ti]` para compartilhar uma pasta do setor de Tecnologia da Informação com permissões específicas.

```ini
[ti]
path = /dados/ti
comment = Pasta do setor de TI
browseable = yes
writable = yes
valid users = @ti
create mask = 0660
...
| `path`             | Caminho da pasta no sistema                                                  |
| `comment`          | Comentário descritivo que aparece no navegador de rede                      |
| `browseable`       | Visível na lista de compartilhamentos                                        |
| `writable`         | Permite escrita na pasta                                                     |
| `valid users = @ti`| Apenas usuários do grupo `ti` poderão acessar                                |
| `create mask`      | Permissões padrão para arquivos criados: leitura/escrita para dono e grupo   |
| `directory mask`   | Permissões padrão para diretórios criados                                   |
| `force group`      | Todos os arquivos serão atribuídos ao grupo `ti`                            |

---

### 🛠️ Exemplo: criar a estrutura e permissões

```bash
sudo mkdir -p /dados/ti
sudo groupadd ti
sudo chown :ti /dados/ti
sudo chmod 770 /dados/ti
```

---

### ➕ Adicionar usuários ao grupo `ti`

```bash
sudo usermod -aG ti joao
sudo usermod -aG ti maria
```

> Os usuários precisam sair e entrar novamente no sistema para aplicar a mudança de grupo.

---

### 🔐 Criar senhas Samba para os usuários
